...
QEMUFLAGS := -machine pc -m 256M -serial stdio -no-reboot -no-shutdown -vga std -display gtk,grab-on-hover=on
...
UEFI_QEMUFLAGS := -machine pc -m 256M -serial stdio -no-reboot -no-shutdown -vga std -display gtk,grab-on-hover=on -device qemu-xhci,id=xhci -device usb-tablet,bus=xhci.0
...
uefi-flash:
	...
	@printf 'SacramentuOS 1.0.1\r\nCopy the CONTENTS of this flash directory to a FAT32 USB drive.\r\nMain system: EFI/BOOT/BOOTX64.EFI\r\n' > $(UEFI_TREE)/README_FIRST_RU.txt
	@printf '1.0.1\r\n' > $(FLASH_SYSTEM)/VERSION.txt
	@printf 'Mouse click/drag F1 Start F2 Explorer F3 Console F4 About F5 Settings F6 Code F7 GPUx help commands fetch calc edit code files show rm note todo gpux theme reboot shutdown\r\n' > $(FLASH_SYSTEM)/TEST_COMMANDS.txt
	@printf 'BOOTX64.EFI contains the native SacramentuOS GOP pixel desktop with windows, Explorer, Console, Code Studio, GPUx, Settings themes, UEFI pointer protocols and raw PS/2 mouse fallback. USB host-controller PCI probing is included; full raw USB HID transfers are not enabled yet. No ISO is required for UEFI boot.\r\n' > $(FLASH_SYSTEM)/WHAT_IS_SYSTEM.txt
...
help:
	...
	@echo "  uefi              build native GOP pixel BOOTX64.EFI from source"
